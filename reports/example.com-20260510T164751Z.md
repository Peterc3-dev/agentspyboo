# AgentSpyBoo Assessment — example.com

**Date:** 2026-05-10T16:47:51.639582778+00:00  
...
Subfinder identified 22,248 subdomains for example.com. However, HTTPx did not find any live hosts among these subdomains.
...
**Mode:** passive  
**Runtime:** 53.1s  
**Plugins fired:** crt-sh, urlscan, wayback  
**Records:** 13 raw, 3 filtered out
...
| `example.com` | urlscan, wayback, crt-sh | — |
| `m.example.com` | crt-sh | — |
| `products.example.com` | crt-sh | — |
| `support.example.com` | crt-sh | — |
| `www.example.com` | urlscan, wayback, crt-sh | — |

### CIDR blocks

_No CIDR blocks discovered (supply `--asn` to enable BGP lookup)._
...
| Plugin | Status | Required env vars | Note |
|--------|--------|-------------------|------|
| `passive-dns` | skipped (key missing) | `SECURITYTRAILS_API_KEY` | paid only (SecurityTrails ~$50/mo min) |
| `reverse-whois` | skipped (key missing) | `VIEWDNS_API_KEY` | free tier available at viewdns.info |
| `apollo` | skipped (key missing) | `APOLLO_API_KEY` | B2B paid plan only |
| `favicon-hash` | skipped (key missing) | `SHODAN_API_KEY` | Shodan 100/mo free, burns fast |
| `shodan` | skipped (key missing) | `SHODAN_API_KEY` | Shodan 100/mo free, burns fast |
| `censys-org` | skipped (key missing) | `CENSYS_API_TOKEN` | Censys Starter+ plan, ~$100 min credits |
| `github-org` | skipped (key missing) | `GITHUB_TOKEN` | free personal token, raises rate limit |
| `reverse-ip` | skipped (key missing) | `VIEWDNS_API_KEY` | free tier; HackerTarget used as fallback when key missing |

_To enable the skipped plugins, set: `APOLLO_API_KEY`, `CENSYS_API_TOKEN`, `GITHUB_TOKEN`, `SECURITYTRAILS_API_KEY`, `SHODAN_API_KEY`, `VIEWDNS_API_KEY`. See `docs/pius-api-keys.md` for free-tier reality._
...
_Dedup folded 22248 raw observations into 1 grouped findings. Disable with `--no-dedup`._

| # | Severity | Type | Targets | Details |
|---|----------|------|---------|---------|
| 1 | ℹ️ info | subdomain | 22248 targets (x22248) | discovered via subfinder |

<details><summary>Finding #1 — 22248 target(s)</summary>
...
- Review the subdomain list for potential manual investigation or further passive enumeration techniques.
